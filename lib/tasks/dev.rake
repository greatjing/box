...
namespace :dev do
  ...
  task :fake_event_and_registrations => :environment do
    event = Event.create!( :status => "public", :name => "meetup", :friendly_id => "meetup" )
    t1 = event.tickets.create!( :name => "guest", :price => 0 )
    t2 = event.tickets.create!( :name => "vip1", :price => 100 )
    t3 = event.tickets.create!( :name => "vip2", :price => 200 )
    ...
    puts "Let's visit http://localhost:3000/admin/events/meetup/registrations"
  end
